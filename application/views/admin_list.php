			...
			if($mdata->num_rows()>0){
				...
				foreach($mdata->result_array() as $rows){
					...
					if($rows['status']==1){
						...
						$button='<a class="btn btn-sm btn-warning" href="'.site_url('admin/suspend/'.trim(base64_encode($rows['id_admin']),'=').'').'">
									<i class="fa fa-ban" aria-hidden="true"></i>
								 </a>';
					}else if($rows['status']==2){
						$status='<span class="label label-warning">Suspend</span>';
						$button='<a class="btn btn-sm btn-warning" href="'.site_url('admin/aktivasi/'.trim(base64_encode($rows['id_admin']),'=').'').'">
									<i class="fa fa-ban" aria-hidden="true"></i>
								 </a>';
					}else{
					...
					echo '<td>
							...
							<a class="btn btn-sm btn-danger" href="#" data-href="'.site_url('admin/delete/'.trim(base64_encode($rows['id_admin']),'=').'').'"  data-toggle="modal" data-target="#confirm-delete" >
