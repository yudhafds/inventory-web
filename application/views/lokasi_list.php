...
<section class="content">
  <div class="row">
	<div class="col-xs-12">
	<a href="<?php echo site_url('lokasi/add');?>">
		  ...
		  <table id="user" class="display table-bordered table-striped table-hover" cellspacing="0" width="100%">
			...
			if($mdata->num_rows()>0){
				...
				foreach($mdata->result_array() as $rows){
					echo '<tr>';
					echo '<td>'.$no.'</td>';
					echo '<td>'.$rows['id_lokasi'].'</td>';
					echo '<td>'.$rows['nama_lokasi'].'</td>';
					echo '<td>'.$rows['keterangan'].'</td>';
					echo '<td>'.$rows['tipe'].'</td>';
					echo '<td>
							<a class="btn btn-sm btn-primary" href="'.site_url('lokasi/edit/'.trim(base64_encode($rows['id_lokasi']),'=').'').'">
								<i class="fa fa-pencil" aria-hidden="true"></i>
							</a>
							<a class="btn btn-sm btn-danger" href="#" data-href="'.site_url('lokasi/delete/'.trim(base64_encode($rows['id_lokasi']),'=').'').'"  data-toggle="modal" data-target="#confirm-delete" >
